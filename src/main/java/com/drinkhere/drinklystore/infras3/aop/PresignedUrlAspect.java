package com.drinkhere.drinklystore.infras3.aop;
...
public class PresignedUrlAspect {
    ...
    private void processPresignedUrl(Object data) {
        for (Field field : data.getClass().getDeclaredFields()) {
            // filePath 필드만 찾아서 처리
            if (field.getType().equals(String.class) && (field.getName().contains("ImageUrl") || field.getName().contains("ImageUrls"))) {
            ...
            else if (field.getType().equals(List.class) && field.getName().contains("imageUrls")) {
                field.setAccessible(true);
                try {
